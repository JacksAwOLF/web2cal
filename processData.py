...
def webreg_json(text):
    ...
    for line in text.split("\n"):
        ...
        # bugfix: class titles with more than 1 line should combine with next line
        if combine:
            x = [lastline[0], lastline[1] + x[0]] + x[1:]
            combine = False
            print("combined line from last line", x)
        elif len(x) == 2 and len(x[1]) > 3:
            combine = True
            lastline = x
            print("class title is 2 lines! skip current one")
            continue
...
def next_day(today):
    ...
    if dd>lenMonth[mm-1]+(1 if leap else 0):
        dd = 1
        mm += 1
        if mm > 12:
            mm = 1
